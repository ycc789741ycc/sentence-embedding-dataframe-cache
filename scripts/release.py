...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--next_version", type=str, help="Either next version number or 'major', 'minor', 'micro', 'alpha', 'rc'"
    )
...
def get_current_version() -> Text:
    ...
    if not version_file_path().is_file():
        raise FileNotFoundError(f"Failed to find version file at {version_file_path().absolute()}")
...
def ask_version() -> Text:
    ...
    if version in PRERELEASE_FLAVORS and not current_version.pre:
        ...
        version = questionary.select(f"Which {version} do you want to release?", choices=choices).ask()
...
def git_current_branch_is_master_or_release() -> bool:
    ...
    return current_branch == "master" or RELEASE_BRANCH_PATTERN.match(current_branch) is not None
...
def generate_changelog(version: Version) -> None:
    ...
    check_call(["towncrier", "build", "--yes", "--version", str(version)], cwd=str(project_root()))
...
def print_done_message_same_branch(version: Version) -> None:
    ...
    print()
    print(f"\033[94m All done - changes for version {version} where committed on this branch \033[0m")
